...
int cpuUsage(int nProcessor, int totalJiffies[MAX_PROCESSORS], int workJiffies[MAX_PROCESSORS]);

int main()
{
    int nProcessor = sysconf(_SC_NPROCESSORS_ONLN);
    ...
    while(1)
    {
    	cpuUsage(nProcessor, totalJiffies1, workJiffies1);
        
        usleep(1000000);

    	cpuUsage(nProcessor, totalJiffies2, workJiffies2);
    	...
    	for(int i{0}; i <= nProcessor; i++)
    	{
    		...
    		if(i == 0)
    		{
    			std::cout << "Total cpu: " << cpuUsagePercentage[i] << "%" << std::endl;
    		}
    		else
    		{
    			std::cout << "cpu" << i << ": " << cpuUsagePercentage[i] << "%" << std::endl;
    		}
    	}
    	std::cout << std::endl;
    }
